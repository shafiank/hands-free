 ...
 *
 * Storage: ~/.handsfree-data/buddy-queue.json  (array of { text, ts })
...
export interface BuddyNotification {
    text:      string;
    ts:        number;
    isError?:  boolean;   // true â†’ buddy shows friendly "Oops" message + Open Chat button
    // â”€â”€ v7 rich notification metadata (backward-compatible: all optional) â”€â”€
 ...
 *
 * @example
 *   pushBuddyNotification('âœ… Task "Send weekly report" completed.');
 *   pushBuddyNotification('ðŸ“§ Email sent to john@example.com.');
 */
...
/** Push an error notification â€” shows friendly message + "Open Chat" button. */
