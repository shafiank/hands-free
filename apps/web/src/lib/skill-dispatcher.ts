/**
 * Hands Free â€” Meta-Agent Skill Dispatcher (Phase 5)
 *
 * Gives the Autopilot runner programmatic, isolated access to EVERY
 * active Hands Free skill without touching the user's foreground UI.
 *
 * Design principles:
 ...
 *  - DETERMINISTIC: Returns a structured DispatchResult â€” never throws
...
export interface SkillContext {
    task:     AgentTask;
    settings: any;       // full HandsFreeSettings from loadSettings()
...
// â”€â”€â”€ Critical Action Detector â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
const SKILL_HANDLERS: Record<string, SkillHandler> = {
    ...
    // â”€â”€ Web Search â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    web_search: async (ctx, params) => {
        ...
        try {
            ...
            const results: string[] = (data.results ?? []).slice(0, 5).map(
                (r: any) => `â€¢ ${r.title}: ${r.content?.slice(0, 200) ?? ''}`,
            );
            const output = `Web search for "${query}":\n${results.join('\n')}`;
            log.success('skill_used', `ðŸ” Web search: "${query}"`, { taskId: ctx.task.id, detail: { resultCount: results.length } });
    ...
    // â”€â”€ Document Generation â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    documents: async (ctx, params) => {
        try {
            ...
            const result = await createDocument({
                ...
                author:   'Hands Free Autopilot',
            });
            log.success('skill_used', `ðŸ“„ Document generated: ${params.filename}`, { taskId: ctx.task.id });
    ...
    // â”€â”€ Network Scanner â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    network_scanner: async (ctx, params) => {
        try {
            const { findHandsFreeInstances, scanLocalNetwork } = await import('@/actions/network-scanner');
            const result = params.mode === 'full' ? await scanLocalNetwork() : await findHandsFreeInstances();
            log.success('skill_used', `ðŸ“¡ Network scan completed (${result.hosts?.length ?? 0} hosts found)`, { taskId: ctx.task.id });
            return { success: result.success, output: `Found ${result.hosts?.length ?? 0} live hosts, ${result.handsfreeInstances?.length ?? 0} Hands Free instances.`, data: result, skillId: 'network_scanner' };
        } catch (e: any) {
            return { success: false, error: e.message, skillId: 'network_scanner' };
        }
    },

    // â”€â”€ Email â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    email: async (ctx, params) => {
        try {
            ...
            log.success('skill_used', `ðŸ“§ Email sent to ${params.to}`, { taskId: ctx.task.id });
    ...
    twitter: async (ctx, params) => {
        try {
            ...
            log.success('skill_used', `ðŸ¦ Tweet posted`, { taskId: ctx.task.id });
    ...
    // â”€â”€ Google Calendar â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    googleCalendar: async (ctx, params) => {
        try {
            ...
            log.success('skill_used', `ðŸ“… Calendar event created: ${params.summary}`, { taskId: ctx.task.id });
    ...
    // â”€â”€ OODA Re-Planner â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    ooda_replan: async (ctx, params) => {
        try {
            ...
            log.info('plan_created', `ðŸ”„ OODA re-plan triggered: ${params.newContext?.slice(0, 80)}`, { taskId: ctx.task.id });
    ...
    // â”€â”€ Internal Sub-Agent Group Chat â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    internal_group_chat: async (ctx, params) => {
        try {
            ...
            log.success('skill_used', `ðŸ¤– Internal group chat: ${agents.join(', ')} deliberated on "${topic.slice(0, 60)}"`, { taskId: ctx.task.id });
...
// â”€â”€â”€ Internal LLM direct caller (exported for use by internal_group_chat) â”€â”€â”€â”€
...
// â”€â”€â”€ Main Dispatch Function â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

/**
 * Dispatch a skill call from the Autopilot runner.
 * ALWAYS runs headless â€” never touches the user's foreground UI.
 *
 * @param skillId   The skill to invoke (must match a key in SKILL_HANDLERS)
 * @param task      The AgentTask being processed
 * @param settings  Full HandsFreeSettings
...
export async function dispatchSkill(
    ...
    try {
        ...
        log.error('skill_used', `âŒ Skill "${skillId}" threw unexpectedly: ${e.message}`, { taskId: task.id });
 ...
 * Uses keyword heuristics â€” the LLM sets explicit [SKILL:xxx] tags when
