...
// Quoty â€” AI-generated inspirational quotes
...
/**
 * Make an HTTP(S) request â€” works with Node.js built-ins only.
 */
  ...
  async execute(input, context) {
    try {
      ...
      const prompt = `Generate one unique, profound, and memorable inspirational quote about "${theme}". `
        + `The quote should feel like it was said by a great thinker but be COMPLETELY ORIGINAL â€” not a real quote. `
      ...
      for (const provId of providerOrder) {
        ...
        if (provId === 'openrouter') {
          ...
          headers['x-title']     = 'Hands Free Quoty';
        }
      ...
      // â”€â”€ Fallback: no working provider â”€â”€
