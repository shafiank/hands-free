...
// Never cache â€” live Telegram message handler
...
// â”€â”€â”€ Core Telegram Processing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function processOneTelegramMessage(
    ...
    try {
        ...
        if (!session) {
            ...
            if (!session) {
                session = await createSession(`Telegram Chat with ${telegramUserName || 'User'}`, 'handsfree');
        ...
        const PERSONA_PROMPTS: Record<string, string> = {
            default: `You are Hands Free, a friendly and capable AI assistant. You help with daily life, work, planning, and creativity. You are direct, helpful, and have a good sense of humor. ${langInstruction}`,
            entrepreneur: `You are Hands Free in Entrepreneur mode. Sharp business advisor for strategy, marketing, finance, and growth. Direct, data-driven, actionable.`,
            family: `You are Hands Free in Family mode. Warm, patient helper for everyday tasks â€” recipes, scheduling, health tips, parenting. Simple and friendly.`,
            coder: `You are Hands Free in Coder mode. Senior software engineer. Clean, efficient code. Best practices.`,
            student: `You are Hands Free in Student mode. Patient tutor. Step-by-step explanations. Examples and encouragement.`,
        };
...
The user "${telegramUserName || 'User'}" is messaging you through Telegram. You have EXACTLY THE SAME capabilities as in the Hands Free Dashboard chat. You are NOT a limited chatbot â€” you are the full Hands Free agent with all tools.

## Your full capabilities (USE THEM!)

You have access to powerful tools that let you interact with the user's computer. If the user asks you to DO something, use the appropriate tool. NEVER say "I can't do that" or "I'm just a text chatbot" â€” that is WRONG.

### Available Tools:
- **create_task** â€” Create tasks and to-dos
- **list_tasks** â€” Show current tasks
- **delete_task** â€” Delete a task
- **schedule_recurring_task** â€” Set up recurring tasks/reminders (e.g. "daily at 9am")
- **list_scheduled_tasks** â€” Show all scheduled tasks
- **delete_scheduled_task** â€” Delete a scheduled task
- **send_telegram_notification** â€” Send a proactive Telegram message
- **create_folder** â€” Create a folder
- **list_files** â€” List files
- **read_file** â€” Read file content
- **write_file** â€” Write/create files
- **delete_file** â€” Delete files
- **execute_command** â€” Run shell commands (PowerShell on Windows)
- **fetch_web_page** â€” Fetch a web page
- **extract_web_text** â€” Extract text from a web page
- **get_workspace_info** â€” Get workspace info
- **get_system_info** â€” Get system specs
- **generate_image** â€” Generate an image with AI (requires Google API key in Settings)
- **generate_video** â€” Generate a video with AI (requires Google API key in Settings)
- **search_gif** â€” Search for GIFs
- **send_gif_telegram** â€” Send a GIF via Telegram
- **generate_voice** â€” Generate a TTS voice message and send via Telegram (always available, no API key needed for basic TTS)
- **analyze_image** â€” Analyze an image sent by the user (returns vision capability status and model guidance)
- **check_capabilities** â€” Check active integrations
- **check_identity** â€” Check onboarding status
- **check_system_status** â€” Check system health

### Rules:
1. ALWAYS use tools when the user wants something DONE (create, schedule, remind, check, etc.)
2. Do NOT simulate or pretend â€” actually execute the tool
...
6. ${langInstruction}
7. Keep Telegram replies short â€” no long markdown walls of text
8. Sending GIFs via Telegram is possible: search_gif + send_gif_telegram
9. Image generation: use generate_image tool â€” the image will be automatically sent to Telegram after generation

You are the same Hands Free as in the Dashboard. Same brain, same tools, same capabilities. Act accordingly.`;

        // â”€â”€ Prepend EXECUTION MANDATE so Telegram uses the same agent behavior as Dashboard â”€â”€
        ...
        const executionMandate = `EXECUTION MANDATE â€” THIS OVERRIDES EVERYTHING BELOW:
...
3. NEVER respond with just text when a tool call would accomplish
   the task. If the user says "create a file" â€” call write_file.
   If the user says "check my email" â€” call list_emails.
   If the user says "search for X" â€” call search_web.
...
Do not say "I will do X" â€” just do X. Every message from you should
...
` : '';
        ...
        // â”€â”€ Vision Provider pre-analysis â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                    ...
                    if (analysisResult.success && analysisResult.description) {
                        ...
                        const userText = typeof rawContent === 'string'
                            ...
                            : (Array.isArray(rawContent)
                                ...
                                : '');
                        const textWithVision = `[ðŸ“· Image analyzed by Vision Provider]\n${analysisResult.description}${userText ? `\n\nUser's message: ${userText}` : ''}`;
                        ...
                        telegramSystemPrompt += `\n\nâ„¹ï¸ The user sent an image. It has been analyzed by the Vision Provider and the description is included in the message above as "[ðŸ“· Image analyzed by Vision Provider]". Respond to the image content naturally.`;
                    }
                } else if (!isLikelyVision && activeModel) {
                    // Main LLM likely non-vision, no Vision Provider fallback â†’ inform user clearly
                    telegramSystemPrompt += `\n\nâš ï¸ VISION NOTE: The active model "${activeModel}" may not support image analysis. If you cannot process the image, tell the user to enable the Vision Provider in Settings â†’ Vision Provider with "Telegram" checked.`;
                }
        ...
        while (loopCount < MAX_LOOPS) {
            ...
            if (signal?.aborted) {
                finalResponse = 'âš ï¸ Request cancelled.';
                break;
            }
            ...
            if (decision.decision === 'error') {
                ...
                if (isVisionError) {
                    finalResponse = 'ðŸ“· Your current model doesn\'t support image analysis directly. Enable the Vision Provider in Settings â†’ Vision Provider and check "Telegram" to analyze images with any LLM.';
                } else {
                    finalResponse = `âš ï¸ Error: ${errMsg}`;
                }
            ...
            if (decision.decision === 'response') {
                ...
                // â”€â”€ Safe Mode reinforcement: if the agent returned a text-only
                ...
                // what they *would* do. â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if (loopCount === 1 && ACTION_WORDS.test(message)) {
                    ...
                    currentMessages.push({
                        role: 'user',
                        content: 'You MUST use tools to complete this task. Do NOT describe what you would do â€” call the appropriate tool NOW. If the task needs approval, return the tool call with requiresConfirmation: true.',
                    } as ChatMessage);
            ...
            if (decision.decision === 'tool') {
                ...
                // â”€â”€ Approval gate â€” any tool needing confirmation? â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                if (needsApproval.length > 0) {
                    ...
                    // Build the approval message â€” text-based (inline keyboards are unreliable)
                    const actionList = descriptions.map((d, i) => `${i + 1}. ${d}`).join('\n');
                    finalResponse = `âš ï¸ Approval needed:\n\nI want to execute:\n${actionList}\n\nReply "yes" to approve or "no" to decline.`;
                ...
                // â”€â”€ Normal tool execution â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        try {
            ...
            await addMemory('short-term', {
                // Do NOT prefix with "[Telegram]" â€” it pollutes the word cloud in dashboard.
...
// â”€â”€â”€ POST Handler â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        // â”€â”€ Security: verify Telegram integration is configured and chat ID is authorized â”€â”€
        ...
        // â”€â”€ Autopilot Approval Commands â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
                ...
                try {
                    ...
                    if (!targetTask) {
                        await sendMessage(tgConfig.botToken, telegramChatId,
                            `âŒ Task \`${taskId}\` not found.`);
                    } else if (approveMatch) {
                        updateTask(taskId, { approvalStatus: 'approved' });
                        await sendMessage(tgConfig.botToken, telegramChatId,
                            `âœ… Task *${targetTask.title}* approved. It will run on the next autopilot tick.`);
                    } else {
                        updateTask(taskId, { approvalStatus: 'rejected', state: 'cancelled', completedAt: Date.now(), blockedReason: 'Rejected via Telegram' });
                        await sendMessage(tgConfig.botToken, telegramChatId,
                            `ðŸš« Task *${targetTask.title}* rejected and cancelled.`);
                    }
                } catch (err: any) {
                    await sendMessage(tgConfig.botToken, telegramChatId,
                        `âš ï¸ Could not process approval: ${err.message}`).catch(() => {});
                }
                return NextResponse.json({ success: true, handled: 'approval_command' });
            }

            // Simple yes/no â€” only applies when exactly 1 autopilot task is awaiting approval
                    ...
                    if (waiting.length === 1) {
                        ...
                        if (taskAge < 5 * 60 * 1000) {
                            if (yesNo === 'yes') {
                                ...
                                await sendMessage(tgConfig.botToken, telegramChatId,
                                    `âœ… Task *${t.title}* approved. It will run on the next autopilot tick.`);
                            } else {
                                updateTask(t.id, { approvalStatus: 'rejected', state: 'cancelled', completedAt: Date.now(), blockedReason: 'Rejected via Telegram' });
                                await sendMessage(tgConfig.botToken, telegramChatId,
                                    `ðŸš« Task *${t.title}* rejected and cancelled.`);
                            }
                            return NextResponse.json({ success: true, handled: 'approval_yesno' });
                        }
                        // Task is stale (> 5 min old) â€” fall through to normal chat
                    }
                    // If 0 or 2+ tasks are waiting, fall through to normal chat
                } catch { /* non-fatal â€” fall through to normal chat processing */ }
            }
        }

        // â”€â”€ Queue Guard â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        // If Hands Free is already processing a Telegram message, queue this one
        ...
        if (telegramQueue.isProcessing) {
            ...
            if (!result.success) {
                // Queue is full â€” notify user without LLM
                await sendMessage(
                    tgConfig.botToken,
                    telegramChatId,
                    `âŒ Message queue is full (max 20 messages). Please wait a moment and try again.`
                );
                return NextResponse.json({ success: false, error: result.error });
            }

            // Notify user their message is queued â€” pure system message, NO LLM call
            await sendMessage(
                tgConfig.botToken,
                telegramChatId,
                `â³ Got it! I'm still working on your previous message. Your message is queued (position ${result.position}).`
            );
            return NextResponse.json({ success: true, queued: true, position: result.position, id: result.id });
        }

        // â”€â”€ Acquire Processing Lock â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const abortController = new AbortController();
        telegramQueue.setProcessing(true, abortController);

        // â”€â”€ Thinking Indicator â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        try {
            await sendMessage(tgConfig.botToken, telegramChatId, 'ðŸ¤” Thinking...');
        } catch { /* non-critical â€” continue even if this fails */ }
        ...
        try {
                ...
                if (raceErr?.message === 'PROCESSING_TIMEOUT') {
                    ...
                    try {
                        await sendMessage(
                            ...
                            `â±ï¸ Sorry, this question took too long to process (> ${PROCESSING_TIMEOUT_MS / 1000}s). Try rephrasing or breaking it into smaller questions.`
                        );
            ...
            // The bot awaits this route's JSON response to send it â€” so only queued messages
            // need explicit sendMessage calls here.

            // â”€â”€ Drain the Queue â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
            ...
            // Always release the lock â€” even if something throws
