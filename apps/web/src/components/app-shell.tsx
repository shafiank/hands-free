...
export default function AppShell({ children }: { children: React.ReactNode }) {
    ...
    // â”€â”€â”€ Email Poll Notifications â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    useEffect(() => {
        const checkEmail = async () => {
            // Skip while tab is hidden â€” prevents fetch errors on OS wake-up
    ...
    // Buddy window: frameless, transparent â€” no sidebar, no shell, no email poll
    ...
    // â”€â”€ Bug 19: Recover from mobile Chrome tab suspension â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Telemetry: fire 'app_start' once per app launch â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    useEffect(() => {
        ...
        try {
            const lastLang = localStorage.getItem('handsfree-telemetry-last-lang');
            if (lastLang !== lang) {
                fetch(`/api/telemetry/ping?event=language&lang=${lang}`).catch(() => {});
                localStorage.setItem('handsfree-telemetry-last-lang', lang);
            }
        } catch {
            // localStorage unavailable â€” send anyway as fallback
            fetch(`/api/telemetry/ping?event=language&lang=${lang}`).catch(() => {});
        }
    }, []);
    ...
    // On mobile we never collapse via state â€” the sidebar is an overlay controlled by mobileOpen
    useEffect(() => {
        ...
        if (pathname !== '/code' && !isMobile) {
            ...
            // We can't know that here â€” so we leave it as-is for manual collapses
        }
    }, [pathname]);

    // Buddy window: fully transparent canvas â€” no sidebar, no topbar, no guards
    if (isBuddy) {
        return (
        ...
        <BootstrapGuard>
            <div className="app-shell">
                ...
                <main id="main-content" className={`main-content ${sidebarCollapsed ? 'main-content-expanded' : ''}`}>
                    {/* Mobile header â€” shrink-0 so it never gets squeezed out */}
                    ...
                    {/* Scrollable content area â€” fills remaining height, scrolls internally */}
                    <div className="main-content-scroll">
                        {/* Email notification banner â€” dashboard only, single row */}
