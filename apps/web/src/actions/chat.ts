...
// ============================================================
// Hands Free Chat Server Actions â€” v2.0
// ============================================================
// Multi-provider support: OpenRouter, OpenAI, Anthropic, Google, Ollama
// Session persistence in .handsfree-sessions/
...
export interface HandsFreeSettings {
    ...
    systemPrompt?: string;
    nativeLanguage?: string; // User's preferred language (e.g. 'en', 'de', 'fr') â€” tells Hands Free which language to use
    // UI settings
    ...
    // Active User Behavior (Hands Free Proactive Mode)
    activeUserBehavior?: {
        enabled: boolean;
    ...
    gifIntegration?: {
        ...
        apiKey: string;
        autoSend: boolean; // Hands Free can proactively send GIFs
    };
    // File System Access (legacy toggle â€” kept for backward compatibility with computer-use.ts)
    // 'workspace' = Hands Free stays inside its own .handsfree-data/workspace sandbox (safe default)
    // 'full'      = Hands Free can read/write anywhere on the local drive (system paths still blocked)
    fileSystemAccess?: 'workspace' | 'full';
    // File Access Mode (new, fine-grained â€” supersedes the simple toggle above)
    ...
    // High-priority tasks always get 2Ã— this value automatically.
    taskTimeoutSeconds?: number; // default: 300
    // Safety Mode â€” controls how dangerous shell/file commands are handled
    // 'safe'         (default) â€” risky commands blocked outright
    // 'advanced'     â€” risky commands ask for Approve/Reject before execution
    // 'unrestricted' â€” no blocking (user takes full responsibility)
    safetyMode?: 'safe' | 'advanced' | 'unrestricted';
    // Autonomous Mode â€” background agent heartbeat processes tasks every 5 minutes
    // When false (default), the heartbeat is stopped and no tasks run proactively.
    isAutonomousMode?: boolean;
    // Google Places API key
    googlePlacesApiKey?: string;
    // Replicate API token (BYOK â€” gives access to 50+ image/video models)
    ...
    localSttUrl?: string;
    // Custom OpenAI-compatible endpoint â€” whether to enable tool/function calling
    // Some local models (llama.cpp, LM Studio, etc.) don't support the tools array
    customEndpointToolCalling?: boolean;
    // Custom endpoint â€” vision support toggle (user declares endpoint accepts image content)
    customEndpointVisionSupport?: boolean;
    // Custom endpoint â€” TTS base URL (optional, for text-to-speech via the custom endpoint)
    customEndpointTtsUrl?: string;
    // Custom endpoint â€” request timeout in seconds (default: 30)
    customEndpointTimeout?: number;
    // Skills â€” enabled/disabled flags for built-in skill modules
    skills?: {
        ...
        // Browser Control skill â€” requires a one-time Chromium download
...
// â”€â”€â”€ Default Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

export interface ChatMessage {
    role: 'user' | 'assistant' | 'system' | 'tool';
    content: string;
    timestamp?: number;
    /** Message origin â€” written to session file so polling can distinguish sources */
...
// â”€â”€â”€ Persona System Prompts â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

const PERSONA_PROMPTS: Record<string, string> = {
    default: `You are Hands Free, a friendly and smart AI assistant. You help with everything â€” daily life, work, planning, creativity. You are direct, helpful, and have a good sense of humor. Keep responses concise but informative. Respond in the user's language.`,
    entrepreneur: `You are Hands Free in Entrepreneur mode. You're a sharp business advisor who helps with strategy, marketing, finance, and growth. You think in terms of ROI, market fit, and execution speed. Be direct, data-driven, and actionable.`,
    family: `You are Hands Free in Family mode. You're a warm, patient helper for everyday family tasks â€” recipes, homework help, scheduling, health tips, parenting advice. Keep things simple and friendly, suitable for all ages.`,
    coder: `You are Hands Free in Coder mode. You're a senior software engineer who writes clean, efficient code. You explain technical concepts clearly, suggest best practices, and help debug issues. Use code blocks with syntax highlighting.`,
    student: `You are Hands Free in Student mode. You're a patient tutor who explains concepts step by step. You encourage curiosity, break down complex topics, and use examples. Help with homework, exam prep, and research.`,
};

// â”€â”€â”€ Settings Load/Save â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

export async function loadSettings(): Promise<HandsFreeSettings> {
    ensureDirs();
    try {
        // Also check legacy settings file
        const legacyFile = path.join(process.cwd(), '.handsfree-settings.json');

        if (fs.existsSync(SETTINGS_FILE)) {
            const raw = JSON.parse(fs.readFileSync(SETTINGS_FILE, 'utf-8'));

            // â”€â”€ Auto-migrate deprecated Google models â”€â”€
            ...
            if (gModel && DEPRECATED_GOOGLE_MODELS[gModel]) {
                console.log(`[Hands Free] Auto-migrating Google model "${gModel}" â†’ "${DEPRECATED_GOOGLE_MODELS[gModel]}"`);
        ...
        if (fs.existsSync(legacyFile)) {
            ...
            const migrated: HandsFreeSettings = {
                ...DEFAULT_SETTINGS,
        ...
        console.warn('[Hands Free] Could not load settings:', e);
    }
    return DEFAULT_SETTINGS;
}

export async function saveAllSettings(newSettings: Partial<HandsFreeSettings>) {
    ensureDirs();
    try {
        const current = await loadSettings();
        const merged: HandsFreeSettings = {
            ...current,
            ...newSettings,
...
// â”€â”€â”€ Provider API Call â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
async function callProvider(
    ...
    // For custom endpoints, normalise the base URL â€” append /v1 if the user didn't include it
...
// â”€â”€â”€ Test Provider Connection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        if (provider === 'ollama') {
            ...
            try {
                ...
                const msg = fetchErr?.name === 'AbortError'
                    ? 'Ollama timeout â€” server not responding. Is it running?'
...
export async function listSessionsByAgent(agentId: string): Promise<{ id: string; title: string; updatedAt: number; messageCount: number }[]> {
    ...
    for (const s of allSessions) {
        try {
            ...
            const sessionAgent = session.agentId || 'handsfree';
...
export async function loadSession(id: string): Promise<ChatSession | null> {
    ...
    try {
        ...
        if (!raw || raw.trim().length === 0) {
            console.warn(`[Hands Free] Session file empty: ${id}`);
            return null;
        }
        const parsed = JSON.parse(raw);
        // Basic validation
        if (!parsed.id || !Array.isArray(parsed.messages)) {
            console.warn(`[Hands Free] Session file invalid structure: ${id}`);
            return null;
        }
        return parsed as ChatSession;
    } catch (e) {
        console.error(`[Hands Free] Could not load session ${id}:`, e);
    }
    return null;
}

// â”€â”€â”€ Active Session Persistence â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function saveSession(session: ChatSession) {
    ...
    // Atomic write: write to .tmp first, then rename â€” prevents corruption on crash/restart
    ...
    try {
        ...
        for (const f of files) {
            try {
                ...
                // Validate required fields â€” skip corrupt or incomplete sessions
                ...
                // Skip this file â€” don't let one bad file break the whole list
                console.warn(`[Hands Free] Skipping corrupt session file: ${f}`);
            }
        }
        return sessions.sort((a, b) => b.updatedAt - a.updatedAt);
    } catch (e) {
        console.error('[Hands Free] listSessions error:', e);
        return [];
    }
}
...
// â”€â”€â”€ Process Chat Message â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
export async function processMessage(
    ...
    options?: {
         ...
         * 'buddy'   â†’ came from the Desktop Buddy overlay (polling detects + displays it)
         * 'browser' â†’ typed directly in the main chat window (already displayed â€” polling skips it)
         * Defaults to 'browser' when omitted so existing callers are unaffected.
         */
        msgSource?: 'browser' | 'buddy';
    }
) {
    // â”€â”€ Slash-command killswitch: /stop  /killswitch  /kill â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    if (trimmed === '/stop' || trimmed === '/killswitch' || trimmed === '/kill') {
        ...
        return {
            success: true,
            response: 'ðŸ›‘ **Killswitch triggered.** All agent activity has been stopped. You can restart via Settings â†’ Autopilot.',
        };
    }
    ...
    try {
        ...
        if (provider !== 'ollama' && provider !== 'custom' && !providerConfig.apiKey) {
            console.warn(`[Hands Free] Setup missing for ${provider}. Triggering fallback...`);
            ...
            for (const fb of fallbackOrder) {
                ...
                if (fbConfig?.apiKey || fbEnvKey || fb === 'ollama') {
                    ...
                    console.log(`[Hands Free] Fallback active: using ${provider} instead.`);
                    fallbackFound = true;
                    break;
                }
            }
        ...
        // Build capabilities context (cached file read â€” no live scan)
        // Injected into every chat so Hands Free always knows what it can do right now.
        let capsContext = '';
        try {
            ...
            if (fs.existsSync(capsFile)) {
                ...
                const lines: string[] = ['## Your active capabilities (use these â€” never guess):'];
        ...
        } catch { /* non-fatal â€” continue without capabilities block */ }
        ...
        console.log(`[Hands Free] Chat â†’ ${provider} (${providerConfig.model})`);
        ...
        console.error('[Hands Free] Chat Error:', error);
