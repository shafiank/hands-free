// Hands Free v6.0.0 â€” Created by Mario Simic â€” handsfree.app
'use strict';
...
function safeWrite(method, ...args) {
  try {
    ...
    if (error && (error.code === 'EPIPE' || error.errno === -4047)) {
      return;
    }
    throw error;
  }
...
const log = (...args) => safeWrite('log', ...args);
const warn = (...args) => safeWrite('warn', ...args);
const error = (...args) => safeWrite('error', ...args);

// â”€â”€â”€ Windows App User Model ID â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Must be set BEFORE app.requestSingleInstanceLock() and BEFORE the app is
// ready. This ensures Windows toast notifications display "Hands Free" instead of
// the Electron app ID (com.squirrel.electron.electron), and the app is
// correctly pinned to the taskbar as "Hands Free" rather than "Electron".
if (process.platform === 'win32') {
  app.setAppUserModelId('HandsFree');
}
...
// Prevent a second Hands Free process from opening. If a second instance tries to
...
// Range starts at 3000 and extends to 3009 so multiple Hands Free instances
...
async function findAvailablePort() {
  for (const port of PREFERRED_PORTS) {
    if (await isPortFree(port)) {
      console.log(`[Hands Free] Port ${port} is available.`);
      return port;
    }
    console.log(`[Hands Free] Port ${port} in use, trying nextâ€¦`);
  }
  throw new Error('No available port found (3000-3009). Close other Hands Free instances and try again.');
}
 ...
 * as real filesystem paths â€” which is required for spawn() to work.
 *
 * app.getAppPath() returns:
 *   dev  â†’  /path/to/project
 *   prod â†’  /path/to/Hands Free.app/Contents/Resources/app.asar   (mac)
 *            C:\Program Files\Hands Free\resources\app.asar        (win)
 */
function getWebDir() {
  if (!app.isPackaged) {
    // Development: resolve relative to electron/main.js  â†’  project root / apps/web
  ...
  // extracts to â€” this is safer than .replace() which breaks if the word
...
/**
 * User data directory â€” ALWAYS in the user's home folder, NEVER inside the
 ...
 *   process.env.HANDSFREE_DATA_DIR  (set by us before spawning the server)
 *   || path.join(os.homedir(), '.handsfree-data')  (fallback for standalone web)
 */
function getDataDir() {
  return path.join(app.getPath('home'), '.handsfree-data');
}

// â”€â”€â”€ Auto-launch IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ipcMain.on('set-auto-launch', (_event, enabled) => {
  ...
  console.log(`[Hands Free] Auto-launch ${enabled ? 'enabled' : 'disabled'}.`);
});

// â”€â”€â”€ Export / Save-dialog IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Used by the Export Backup feature so it can save the ZIP to a user-chosen
// location using a native save dialog â€” rather than relying on the browser's
...
ipcMain.handle('show-save-dialog', async (_event, options) => {
  ...
  console.log('[Hands Free] show-save-dialog result:', result);
...
ipcMain.handle('copy-file', (_event, src, dst) => {
  ...
  try {
    ...
    console.log(`[Hands Free] copy-file: ${src} â†’ ${dst}`);
    return { success: true };
  } catch (e) {
    console.error('[Hands Free] copy-file failed:', e.message);
    return { success: false, error: e.message };
  }
});

// â”€â”€â”€ Persistent Settings â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// instead we read/write plain JSON via fs â€” safe to call any time after ready.
...
function writeSettings(patch) {
  try {
    ...
    console.error('[Hands Free] Could not write settings:', e.message);
  }
}
...
// â”€â”€â”€ Splash Error â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function showSplashError(message) {
  console.error('[Hands Free] STARTUP ERROR:', message);
  if (!splashWindow || splashWindow.isDestroyed()) return;
  // JSON.stringify produces a safe JS string literal (handles quotes, newlines â€¦)
...
// â”€â”€â”€ Splash Status â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// â”€â”€â”€ First-launch node_modules extraction (Windows fast-install) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
//
// macOS builds ship node_modules/ directly (no tar.gz) â€” this function is a
...
async function ensureNodeModules() {
  ...
  // macOS: no tar.gz present â€” node_modules is already there as a folder.
  if (!fs.existsSync(tarPath)) return;

  // Already extracted on a previous launch â€” nothing to do.
  if (fs.existsSync(nodeModsPath)) {
    console.log('[Hands Free] node_modules already extracted â€” skipping first-time setup.');
    return;
  }

  // â”€â”€ First launch on Windows â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  console.log('[Hands Free] First-time setup: extracting node_modules.tar.gz â€¦');
  console.log('[Hands Free] tar source :', tarPath);
  console.log('[Hands Free] tar dest   :', webResDir);
  updateSplashStatus('First-time setupâ€¦ (~20 seconds)');
  ...
  await new Promise((resolve, reject) => {
    ...
    proc.stderr && proc.stderr.on('data', (d) => {
      console.error('[Hands Free] tar stderr:', d.toString().trim());
    });

    proc.on('error', (err) => {
      reject(new Error(`tar not found â€” ${err.message}. Ensure Windows 10+ is installed.`));
    });
  ...
  // Free ~20-40 MB â€” archive is no longer needed once extracted.
  try {
    fs.rmSync(tarPath);
    console.log('[Hands Free] node_modules.tar.gz removed after extraction.');
  } catch (e) {
    console.warn('[Hands Free] Could not remove tar.gz (non-fatal):', e.message);
  }

  console.log('[Hands Free] First-time setup complete.');
  updateSplashStatus('Starting serverâ€¦');
}
...
async function loadLocalUrl(window, url) {
  ...
  if (!app.isPackaged) {
    try {
      ...
      // Ignore cache clear failures in development.
    }
  }
...
async function createWindow() {
  ...
  log('[Hands Free] Loading main window:', targetURL);
  loadLocalUrl(mainWindow, targetURL).catch(err => {
    error('[Hands Free] main window load failed:', err.message);
  });
  ...
  // â€¢ Internal localhost links (e.g. /chat, /settings) â†’ navigate the main
  //   window in-place so they never accidentally open a second Electron window.
  // â€¢ External URLs â†’ hand off to the OS default browser via shell.openExternal.
  // â€¢ Everything else â†’ deny (no new windows ever open inside Electron).
  mainWindow.webContents.setWindowOpenHandler(({ url }) => {
    try {
      ...
      if (isLocalhost) {
        ...
        setImmediate(() => {
          if (mainWindow) {
            loadLocalUrl(mainWindow, url).catch(err => {
              error('[Hands Free] internal navigation failed:', err.message);
            });
          }
        });
      } else {
      ...
      // Malformed URL â€” deny silently
    }
    return { action: 'deny' };
  });

  // â”€â”€ Context menu: enable copy/paste/cut/selectAll in editable fields â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // â”€â”€ Desktop Buddy: show/hide with main window visibility â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  mainWindow.on('minimize', () => {
    log('[Hands Free] Main window minimized â€” desktopBuddyEnabled:', desktopBuddyEnabled, 'â€” attempting to show buddy...');
    if (desktopBuddyEnabled) showBuddyWindow();
  });
  mainWindow.on('hide', () => {
    log('[Hands Free] Main window hidden â€” desktopBuddyEnabled:', desktopBuddyEnabled, 'â€” attempting to show buddy...');
    if (desktopBuddyEnabled) showBuddyWindow();
  });
  mainWindow.on('restore', () => {
    log('[Hands Free] Main window restored â€” hiding buddy.');
    hideBuddyWindow();
  });
  mainWindow.on('show', () => {
    log('[Hands Free] Main window shown â€” hiding buddy.');
    hideBuddyWindow();
  });
}

// â”€â”€â”€ Desktop Buddy Window â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
function createBuddyWindow() {
  if (buddyWindow && !buddyWindow.isDestroyed()) return;

  const { width, height } = screen.getPrimaryDisplay().workAreaSize;
  log('[Hands Free] Screen workAreaSize:', width, 'x', height);

  // â”€â”€ Positioning guard: clamp so the window is always fully on-screen â”€â”€â”€â”€â”€â”€
  const WIN_W    = 210;  // narrower â€” eliminates dead whitespace left of gecko
  const WIN_H    = 400;
  ...
  log('[Hands Free] Buddy window position â†’ x:', posX, 'y:', posY);
  ...
  buddyWindow = new BrowserWindow({
    ...
    frame: false,
    transparent: true,
    backgroundColor: '#00000000',   // fully transparent â€” no black flash
    alwaysOnTop: true,
    skipTaskbar: true,
    hasShadow: false,
    resizable: false,
    movable: true,
    focusable: true,
    show: false,   // hidden until page is loaded â€” prevents blank transparent flash on first minimize
    webPreferences: {
      preload: path.join(__dirname, 'preload.js'),
      nodeIntegration: false,
      contextIsolation: true,
  ...
  // Use the same dynamic PORT as the main window â€” never hardcode 3000
  const buddyURL = `http://localhost:${PORT}/buddy`;
  log('[Hands Free] Loading buddy window:', buddyURL);
  buddyWindow.loadURL(buddyURL);
  // Use 'floating' level â€” sits above normal app windows on both Windows and macOS
  ...
  buddyWindow.on('closed', () => {
    log('[Hands Free] Buddy window closed.');
    buddyWindow = null;
  });
}
...
// â”€â”€â”€ Next.js Server â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
async function startServer() {
  ...
  console.log('[Hands Free] Web dir :', WEB_DIR);
  console.log('[Hands Free] Data dir:', DATA_DIR);
  console.log('[Hands Free] Port    :', PORT);
  ...
  const serverEnv = {
    ...
    HANDSFREE_DATA_DIR: DATA_DIR,   // Injected for Next.js to read
    // Passed so server actions (e.g. startTelegramBot) can find telegram-bot.js,
    // whatsapp-bot.js, etc. â€” in the packaged build, process.cwd() inside the
    // Next.js standalone server resolves to the standalone dir, not apps/web.
    HANDSFREE_WEB_DIR: WEB_DIR,
  ...
  if (!app.isPackaged) {
    // â”€â”€ Development â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // â”€â”€ Production â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    const standaloneDir = path.join(WEB_DIR, '.next', 'standalone');
    const serverScript = path.join(standaloneDir, 'server.js');

    // â”€â”€ Diagnostic logging â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    console.log('[Hands Free] Standalone dir  :', standaloneDir);
    console.log('[Hands Free] Server script   :', serverScript);
    console.log('[Hands Free] Server exists   :', fs.existsSync(serverScript));
    console.log('[Hands Free] Window will load: http://localhost:' + PORT);

    // â”€â”€ Guard: abort if server.js is missing â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    // nothing behind it â€” on Windows this renders as a directory listing.
    if (!fs.existsSync(serverScript)) {
      showSplashError('Server files missing â€” please reinstall Hands Free.');
      setTimeout(() => app.quit(), 6000);
      return;
    }

    // â”€â”€ macOS: use the Helper executable to avoid a "exec" cube in the Dock â”€â”€
    // On macOS the Electron binary is a full Cocoa app â€” spawning it directly
    ...
    // Dock.  The packaged .app bundle ships a sibling "Hands Free Helper.app" whose
    ...
    if (process.platform === 'darwin' && app.isPackaged) {
      const helperPath = process.execPath.replace(
        /\/MacOS\/[^/]+$/,
        '/Frameworks/Hands Free Helper.app/Contents/MacOS/Hands Free Helper'
      );
      if (fs.existsSync(helperPath)) {
        serverExec = helperPath;
        console.log('[Hands Free] Using Helper executable for server:', helperPath);
      } else {
        console.warn('[Hands Free] Helper executable not found, falling back to main binary:', process.execPath);
      }
    }
    ...
    serverProcess = spawn(serverExec, [serverScript], {
      ...
      env: {
        ...serverEnv,
        ELECTRON_RUN_AS_NODE: '1', // Required: makes Hands Free.exe behave as Node.js
  ...
  // â”€â”€ Stdout / ready detection â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  serverProcess.stdout.on('data', (data) => {
    const msg = data.toString();
    log(`[Hands Free Server] ${msg.trim()}`);
    ...
    if (
      ...
      serverReady = true;
      log('[Hands Free] Server ready â€” opening window.');
      createWindow();
      ...
      if (desktopBuddyEnabled) {
        setTimeout(() => {
          if (!buddyWindow || buddyWindow.isDestroyed()) {
            log('[Hands Free] Pre-creating buddy window (hidden)...');
            createBuddyWindow();
          }
  ...
  // Collect stderr for diagnostics â€” shown in splash if server exits early
  let stderrBuffer = '';
  serverProcess.stderr.on('data', (data) => {
    const msg = data.toString().trim();
    // Next.js writes some normal startup logs to stderr â€” don't treat as fatal
    error(`[Hands Free Server] ${msg}`);
    // Keep only the last 300 chars for the error splash (enough to diagnose)
    stderrBuffer = (stderrBuffer + '\n' + msg).slice(-300).trim();
  });

  serverProcess.on('close', (code) => {
    log(`[Hands Free] Server process exited (code ${code})`);
    if (!serverReady) {
      // Server died before the app window ever opened â€” show a human-readable
      ...
      const hint = stderrBuffer
        ...
        : 'Check that no other Hands Free instance is running.';
      ...
      // window goes white and the user is stuck â€” nothing responds because the
      // Next.js backend is gone.  Show a dialog offering to restart Hands Free.
      console.error('[Hands Free] Server exited unexpectedly after startup â€” prompting user to restart.');
      const win = mainWindow && !mainWindow.isDestroyed() ? mainWindow : null;
      const dialogOpts = {
        type: 'error',
        title: 'Hands Free â€” Server Stopped',
        message: 'The Hands Free server stopped unexpectedly.',
        detail: 'This can happen if the background helper process is force-quit.\nRestart Hands Free to continue.',
        buttons: ['Restart Hands Free', 'Quit'],
        defaultId: 0,
        cancelId: 1,
      };
  ...
  // â”€â”€ Fallback timer â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  // If no ready signal arrives within 45 s, open anyway rather than hang.
  setTimeout(() => {
    if (!serverReady) {
      console.warn('[Hands Free] Server ready signal timed out â€” opening window anyway.');
      serverReady = true;
      createWindow();
    }
  }, 45_000);
...
// â”€â”€â”€ Telemetry (anonymous, opt-in only) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
function sendTelemetry(event, extra) {
  try {
    ...
    // Fire-and-forget via https.request â€” never blocks the app
    const url = new URL('https://handsfree.app/api/collect.php');
    const opts = {
...
app.on('ready', async () => {
  ...
  console.log('[Hands Free] Desktop Buddy enabled (persisted):', desktopBuddyEnabled);
  ...
  try {
    ...
    console.error('[Hands Free] Failed to start server:', err.message);
...
app.on('window-all-closed', () => {
  // Intentionally do NOT quit here â€” Hands Free lives in the tray
});
...
// â”€â”€â”€ Open-Chat IPC (from Buddy window "Open Chat â†’" button) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Shows the main Hands Free window and navigates directly to the chat route.
ipcMain.on('open-chat', () => {
  if (!mainWindow || mainWindow.isDestroyed()) return;
  mainWindow.show();
  mainWindow.focus();
  // Navigate to /chat â€” loadURL triggers Next.js routing cleanly
  mainWindow.loadURL(`http://localhost:${PORT}/chat`).catch(err => {
    console.error('[Hands Free] open-chat navigation failed:', err.message);
  });
});

// â”€â”€â”€ Desktop Buddy IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ipcMain.on('set-desktop-buddy', (_event, enabled) => {
  ...
  console.log('[Hands Free] Desktop Buddy set to:', desktopBuddyEnabled);
  if (desktopBuddyEnabled) {
...
// â”€â”€â”€ Execute Custom Skill IPC â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Forwards skill execution requests from the renderer to the Next.js backend.
// Called via window.handsfree.executeSkill(skillId, args) from any renderer.
ipcMain.handle('execute-skill', async (_event, skillId, args) => {
  try {
    ...
    if (!res.ok) {
      console.error(`[Hands Free] execute-skill (${skillId}) HTTP ${res.status}:`, data);
    }
    return data;
  } catch (e) {
    console.error(`[Hands Free] execute-skill (${skillId}) failed:`, e.message);
    return { error: e.message };
  }
});

// â”€â”€â”€ Local bug report fallback â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
ipcMain.handle('save-bug-report', (_event, payload) => {
  try {
    ...
    console.log('[Hands Free] Bug report saved locally:', reportPath);
    return { ok: true };
  } catch (e) {
    console.error('[Hands Free] save-bug-report failed:', e.message);
